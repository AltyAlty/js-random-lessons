...
function generalListener(event) { /*Когда срабатывает какое-то событие, в вызываемую функцию отправляется объект
"event", содержащий информацию об этом событии. Свойство "currentTarget" этого объекта означает элемент, с которым
произошло какое-то событие. То есть здесь, когда мы будем кликать на какую-то кнопку, то будем получать "innerHTML" этой
кнопки, что будет содержать "+", "-", "*" или ":". Это мы можем использовать для указания математической операции.*/
...
function calculateSomething(operation) {
    /*"Number.isInteger()" проверяет является ли что-то числом.*/
    if (Number.isInteger(parseInt(inputOneElement[0].value)) && Number.isInteger(parseInt(inputTwoElement[0].value))) {
        switch (operation) {
            case '+':
                /*"Number()", как и "parseInt()", приводит строку к числу, но для проверки выше "Number()" не
                подойдет.*/
