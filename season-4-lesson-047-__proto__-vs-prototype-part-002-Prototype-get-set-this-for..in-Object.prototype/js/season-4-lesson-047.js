...
отсутствует, то JavaScript автоматически берет его из прототипа. В программировании такой механизм называется 
...
/*Свойство "__proto__" — это исторически обусловленный геттер/сеттер для "[[Prototype]]". Свойство "__proto__" — не то 
же самое, что внутреннее свойство "[[Prototype]]". Свойство "__proto__" немного устарело, оно существует по 
историческим причинам. Современный JavaScript предполагает, что мы должны использовать функции 
"Object.getPrototypeOf()"/"Object.setPrototypeOf()" вместо того, чтобы получать/устанавливать прототип. По спецификации
свойство "__proto__" должен поддерживаться только браузерами, но по факту все среды, включая серверную, поддерживают 
его.*/
...
/*Прототипы никак не влияют на this. Неважно, где находится метод: в объекте или его прототипе. При вызове метода 
this — это всегда объект перед точкой.*/
let obj10 = {
    a: 'a',
    b: 'b',
...
появляется в цикле "for..in" так, как у него внутренний флаг "enumerable" стоит false, как и у других свойств 
"Object.prototype". 

Почти все остальные методы, получающие ключи/значения, такие как "Object.keys", "Object.values" и другие – игнорируют 
унаследованные свойства. Они учитывают только свойства самого объекта, не его прототипа.*/

/*-------------------------------------------------------------------------------------------------------------------*/

/*С точки зрения производительности, для современных движков неважно, откуда берется свойство – из объекта или из 
прототипа. Они запоминают, где было найдено свойство, и повторно используют его в следующем запросе.*/