...
/*Метод "JSON.stringify()" берет объект и преобразует его в строку. Полученная строка формата JSON называется 
...
let map01 = new Map();
map01.set('1', 'str1');
console.log(JSON.stringify(map01)); // {}

console.log('--------------------------------------');

/*-------------------------------------------------------------------------------------------------------------------*/

/*Метод "JSON.stringify()" пропускает некоторые специфические свойства объектов JavaScript:
1) Методы.
...
/*Метод "JSON.stringify()" имеет два дополнительных параметра: "replacer" и "space". "Replacer" это массив свойств для 
кодирования или функция соответствия по типу "function(key, value)". "Space" это дополнительное пространство (отступы), 
используемое для форматирования.*/
...
console.log(JSON.stringify(obj07, ['b', 'c', 'd', 'cc', 'a'])); // "{"b":"b","c":[{"cc":"c1"},{"cc":"c2"}],"d":{"a":"a"}}"

/*Если вторым параметром передать функцию в метод "JSON.stringify()", то эта функция будет вызываться для каждой пары 
ключ/значение, и она будет должна возвращать замененное значение, которое будет использоваться вместо исходного, или 
undefined, чтобы пропустить значение. Эта функция получает каждую пару ключ/значение, включая вложенные объекты и 
элементы массива. И она применяется рекурсивно. Значение "this" внутри этой функции – это объект, который содержит 
текущее свойство.*/
console.log(
    JSON.stringify(
        obj07,
...
/*Первому вызову передается специальный "объект-обертка": { "": obj07 }. То есть первая пара (key, value) имеет пустой 
ключ, а значением является целевой объект в общем. Вот почему первая строка из примера выше будет ":[object Object]". 
Благодаря этому можно сделать, чтобы была возможность проанализировать и заменить/пропустить даже весь объект целиком, 
если это необходимо.*/
...
/*Третий аргумент в метода "JSON.stringify()" – это количество пробелов, используемых для удобного форматирования. 
