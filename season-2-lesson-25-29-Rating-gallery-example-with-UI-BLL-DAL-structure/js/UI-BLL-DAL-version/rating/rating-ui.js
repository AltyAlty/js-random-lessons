...
function RatingUI() {
    ...
    /*Метод, который отрисовывает виджет рейтинга на странице. В качестве параметра принимает класс HTML-элемента, в
    котором необходимо отрисовать виджет рейтинга.*/
    this.render = function (HTMLElementClass) {
        ...
        /*Востанавливаем из уровня BLL значение рейтинга. Берем значение рейтинга из уровня BLL и пробегаемся по каждому
        HTML-элементу, отображающему звездочку рейтинга, индекс которого меньший или равный значению рейтинга из уровня
        BLL, и для каждого такого HTML-элемента делаем звездочку активной.*/
        ...
        this._starsElements.forEach((star, index) => {
            star.addEventListener('click', () => {
                /*Сохраняем индекс звездочки рейтинга, на которую кликнули, что будет также являться текущим значением
                рейтинга.*/
                let clickedStarIndex = index;

                /*Пробегаемся по всему массиву со звездочками рейтинга, и звездочки, имеющие индекс меньший или равный
                индексу звездочки рейтинга, на которую кликнули, делаем активными, а остальные звездочки неактивными.*/
