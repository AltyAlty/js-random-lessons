            ...
            for (let i = 0; i < world.worldGrid.length; i++) {
                for (let j = 0; j < world.worldGrid[i].length; j++) {
                    if (
                        ...
                        if (this.checkPixelCollisionLeftRightDownUp(predictedHorizontalPosition.x, predictedHorizontalPosition.y,
                            predictedHorizontalPosition.width, predictedHorizontalPosition.height) && this.currentSpeedX > 0) { // ⇒

                            console.log('Checking RIGHT');

                            while (this.checkPixelCollisionLeftRightDownUp(predictedHorizontalPosition.x, predictedHorizontalPosition.y,
                                predictedHorizontalPosition.width, predictedHorizontalPosition.height)
                            ) {
                        ...
                        if (this.checkPixelCollisionRightLeftDownUp(predictedHorizontalPosition.x, predictedHorizontalPosition.y,
                            predictedHorizontalPosition.width, predictedHorizontalPosition.height) && this.currentSpeedX < 0) { // ⇐

                            console.log('Checking LEFT');

                            while (this.checkPixelCollisionRightLeftDownUp(predictedHorizontalPosition.x, predictedHorizontalPosition.y,
                                predictedHorizontalPosition.width, predictedHorizontalPosition.height)
                            ) {
                                predictedHorizontalPosition.x -= Math.sign(this.currentSpeedX);
                            };
            ...
            for (let i = 0; i < world.worldGrid.length; i++) {
                for (let j = 0; j < world.worldGrid[i].length; j++) {
                    if (
                        ...
                        if (this.checkPixelCollisionLeftRightDownUp(predictedVerticalPosition.x, predictedVerticalPosition.y,
                            predictedVerticalPosition.width, predictedVerticalPosition.height) && this.downwardForce < 0) { // ⇑

                            console.log('Checking UP');

                            while (this.checkPixelCollisionLeftRightDownUp(predictedVerticalPosition.x, predictedVerticalPosition.y,
                                predictedVerticalPosition.width, predictedVerticalPosition.height)
                            ) {
                        ...
                        if (this.checkPixelCollisionLeftRightUpDown(predictedVerticalPosition.x, predictedVerticalPosition.y,
                            predictedVerticalPosition.width, predictedVerticalPosition.height) && this.downwardForce > 0) { // ⇓

                            console.log('Checking DOWN');

                            while (this.checkPixelCollisionLeftRightUpDown(predictedVerticalPosition.x, predictedVerticalPosition.y,
                                predictedVerticalPosition.width, predictedVerticalPosition.height)
                            ) {
                                predictedVerticalPosition.y -= Math.sign(this.downwardForce);
                            };
