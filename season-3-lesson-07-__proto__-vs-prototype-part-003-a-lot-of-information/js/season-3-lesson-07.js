...
/*"prototype" и "__proto___" это свойства объектов. "prototype" каких-то одних объектов может быть равен "__proto___"
каких-то других объектов.*/

/*-------------------------------------------------------------------------------------------------------------------*/

/*"__proto___" это почти всегда какой-то объект. "__proto___" есть у всех объектов. Даже у чисел и строк!*/
let objZero = {};
...
/*"__proto___" разных по типу объектов - разные, а "__proto___" одинаковых по типу объектов - одинаковые. Любой объект в
JS создается на самом деле с помощью классов или функций-конструкторов.*/
let objOne = {};
let objTwo = {};
...
конструкторы. Чтобы понять, что стоит за каким-либо "__proto___", нужно знать с помощью какой функции-конструктора или
класса был создан какой-то объект, "__proto___" которого нас интересует. Стрелочная функция не является
функцией-конструктором, то есть мы можем конструировать объекты при помощи классов или ключевого слова "function", но не
при помощи стрелочных функций. "prototype" используется при создании новых объектов оператором "new".*/
...
/*Любой класс или функция имеет "prototype". Каждый "prototype" это независимый объект со свойствами и методами.
"__proto___" любого объекта ссылается на "prototype" функции-конструктора или класса, при помощи которого этот объект
был создан. "__proto___" нужен для того, чтобы объект связался с "prototype" своего родителя. "prototype" по умолчанию
является объектом с единственным свойством "constructor", которое ссылается на саму функцию или класс.*/
class ClassTwo { };
...
/*Если мы пытаемся прочитать какое-либо свойство у какого-либо объекта, либо вызвать у него какой-либо метод, и при этом
указанного свойства или метода у этого объекта нет, то объект будет искать их через свойство "__proto__", то есть через
ссылку на свойство "prototype" своего родителя. Поэтому при создании функций-конструкторов методы лучше указывать в
"prototype".*/
let objFour = { a: 'abc' };
...
/*В данном случае метод "showA()" будем браться по ссылке из "prototype" общего родителя двух объектов, но контекст
"this" для каждого объекта будет разным.*/
someFuncFiveObjZero.showA(); // 0
someFuncFiveObjOne.showA(); // 1

/*Но лучше использовать классы, чем "prototype" у функций-конструкторов. Метод "showA()" все равно в итоге прицепится к
"prototype".*/
class SomeClassTwo {
    constructor(a) {
        this.a = a;
...
/*"prototype" функций-конструкторов можно модифицировать, чтобы дочерние элементы могли иметь бОльший функционал.*/
...
/*"prototype" ссылается на объект, внутри которого есть свойство "constructor" со значением в виде ссылки на сущность, у
которой было запрошено свойство "prototype".*/
/*"__proto__" ссылается на объект, функцию или класс, от которых была создана сущность, у которой было запрошено
свойство "__proto__".*/
console.log('29----------------------------------------------------------------------------------------------------');
