...
/*В JS любые текстовые данные являются строками. Не существует отдельного типа "символ". Внутренний формат для строк 
...
/*Обратные кавычки также позволяют задавать "шаблонную функцию" перед первой обратной кавычкой. Используемый синтаксис: 
"func`string`". Автоматически вызываемая функция func получает строку и встроенные в нее выражения и может их 
обработать. Если перед строкой есть выражение, то шаблонная строка называется "теговым шаблоном".*/
let a = 1;
let b = 2;

function doSomething1(strings, a, b) {
    console.log(strings); // [ 'A ', ' and B ', '' ]
    console.log(strings[0]); // 'A '
    console.log(strings[1]); //  and B 
...
/*Также есть более старый метод "charAt()", который работает также, как и метод "at()", но работает только со
строками, когда метод "at()" можно использовать с любыми перебираемыми объектами, а также метод "charAt()" не 
принимает отрицательные значения в виде параметра.*/
...
/*Метод "indexOf()" находит позицию в строке, с которой начинается какая-то подстрока. Если ничего не будет найдено,
то вернется -1. Есть второй необязательный параметр, который указывает с какой позиции надо начинать поиск.*/
let string10 = 'aabbcc';
...
/*Оператор побитовый НЕ "~" преобразует число в 32-разрядное целое со знаком. Дробная часть отбрасывается. Затем все 
биты числа инвертируются. То есть любое число превращается в "-(n+1)".*/
let string13 = 'aac';
...
/*Метод "includes()" говорит true если есть подстрока в строке. Есть второй необязательный параметр, который указывает 
с какой позиции надо начинать поиск.*/
let string14 = 'aac';
...
console.log(string15.slice(-3, -1)); // 'cd', с 1-й позиции справа до 3-й позиции справа (не включительно)
...
console.log(string15.substr(0, 6)); // 'abcde'
console.log(string15.substr(2)); // 'cde'
...
/*Для "правильного" сравнения строк внутри разных языков можно использовать метод "localeCompare()". Если строка, у
которой вызывается метод, меньше строки в параметре, то вернется -1. Если строка, у которой вызывается метод, больше 
строки в параметре, то вернется 1. Если же строки равны, то вернется 0. У этого метода есть 2 дополнительных параметра. 
Первый позволяет указать язык (по умолчанию берется из окружения). Второй определяет дополнительные правила, такие как 
чувствительность к регистру, а также следует ли учитывать различия между "a" и "á".*/

let string17 = 'aabbd';
let string18 = 'aabbdcdd';
console.log(string17.localeCompare(string18)); // -1
console.log(string18.localeCompare(string17)); // 1
