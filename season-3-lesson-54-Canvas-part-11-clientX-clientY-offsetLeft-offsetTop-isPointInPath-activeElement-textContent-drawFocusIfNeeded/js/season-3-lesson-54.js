...
redraw(); /*Вроде как лишнее так, как после загрузки страницы срабатывает событие "focus" на document, что 
вызывает функцию "redraw()".*/
...
function handleClick(event) {
    ...
    клиентской области приложения, на которой произошло событие (в отличие от координат внутри страницы). Для 
    примера, нажатие в верхнем левом углу клиентской области будет всегда приводить к событию со значением свойства 
    "clientX" равным 0, независимо от того, прокручена страницы по горизонтали или нет.*/
    /*Свойство "MouseEvent.clientY" свойство доступно только для чтения и является вертикальной координатой в 
    пределах клиентской области приложения, на которой произошло событие (в отличие от координат внутри страницы). 
    Для примера, нажатие в верхнем левом углу клиентской области будет всегда приводить к событию со значением 
    свойства "clientY" равным 0, независимо от того, прокручена страницы по вертикали или нет.*/
    ...
    /*Свойство "HTMLElement.offsetTop" доступно только для чтения и возвращает расстояние от верхней внешней 
    границы элемента до верхней внутренней границы ближайшего родительского элемента.*/
    ...
    drawButton(button1, 20, 20);
    /*Метод "isPointInPath()" сообщает, содержится ли указанная точка в текущем пути. Первым параметром можно
    конкретно указать какой путь использовать для проверки. Последним параметром можно указать алгоритм, 
    по которому будет определяться находится ли точка внутри или снаружи пути.*/
    ...
    /*Если в первом "if" было "true", то после срабатывания события "focus" на кнопке (и на document из-за
    погружения), то управления программы здесь приостановится и запустится функция "redraw()" из-за
    срабатывания события "focus" на document. После работы функции "redraw()" программа продолжит работать
    с этого места, где была остановка.*/
...
function drawButton(el, x, y) {
    ...
    /*Метод "drawFocusIfNeeded()" рисует фокус вокруг текущего или заданного пути, если указанный элемент 
    находится в фокусе.*/
