...
/*Чтобы вызвать при помощи функции "setTimeout()" другую функцию, которая принимает какие-то параметры,
для последней функции нужно создать функцию-обвертку, внутри которой будет вызываться нужная нам
функция с нужными параметрами. Далее эту функцию-обвертку мы должны поместить в функцию "setTimeout()".*/
...
function makeItInvisible(element) {
    if (element[0] != null) {
        element[0].classList.remove(visibleClass); /*Таким образом мы удаляем какой-то отдельный класс из списка классов у HTML-элемента.*/
...
/*Функция должна работать только с теми переменными, которые объявлены в теле этой функции. 
Если функции нужно использовать какие-либо данные вне ее тела, то такие данные должны передаваться 
в эту функцию как параметр.*/
...
function shrinkSomething(element) { /*Функция для плавного скрытия HTML-элемента.*/
    if (element[0] != null) {
        element[0].classList.add(hidingClass); /*Таким образом мы добавляем какой-то отдельный класс к списку классов у HTML-элемента.*/
    };
};
...
function deleteSomethingByValue(someArray, someValue) {
    for (var i = 0; i < someArray.length; i++) { /*Пробегаемся по нашему массиву.*/
        if (someArray[i].value === someValue) { /*Если какой-то элемент массива имеет определенное значение 
            атрибута "value", то удаляем его.*/
            someArray[i].remove();
        };
    };
};

setTimeout(function () { /*Таким образом можно передавать в функцию "setTimeout()" функцию с указанием параметров для нее.*/
...
/*Здесь для каждого элемента массива "someNumberArray" применяем одну из функций 
второго массива "[func1, func2, func3]", указывая параметр "command".*/
...
/*Здесь для каждого элемента массива "someNumberArray" применяем каждую из функций 
второго массива "someFuncArray".*/
