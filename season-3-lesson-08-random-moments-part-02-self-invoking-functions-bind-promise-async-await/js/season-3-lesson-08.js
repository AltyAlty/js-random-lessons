...
/*
Прототип, если кратко, то это объект.
Когда мы создаем новый объект при помощи функции-конструктора, то внутри этой функции-конструктора как бы
неявно создается новый объект ("this" будет являться ссылкой на этот объект), и возвращается этот объект.
*/

/*-------------------------------------------------------------------------------------------------------------*/

/*Анонимную самовызывающуюся функцию можно использовать для объявления каких-нибудь переменных, которые
не будут засорять глобальную область видимости. То есть все, что находится в этой функции, после того как она
отработает умирает.*/
...
/*В данном случае самовызывающаяся анонимная функция вернет другую функцию, которой нужен будет для работы
контекст этой самовызывающейся функции (там будет искаться "a"). Поэтому все, что находится внутри этой
самовызывающейся функции уничтожено не будет, когда эта функция будет отработана, то есть когда переменная
"someFuncZero" сохранит в себе возвращеную функцию.*/
