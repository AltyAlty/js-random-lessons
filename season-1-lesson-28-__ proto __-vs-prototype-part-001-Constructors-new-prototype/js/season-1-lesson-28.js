...
функциями-конструкторами. Чтобы избежать ситуацию, описанную в начале, методы конструктора нужно отдельно указывать при
помощи этого свойства "prototype". В таком случае у созданных объектов уже не будет методов их конструктора, но эти
объекты смогут пользоваться этими метода, используя их из прототипа конструктора благодаря замыканию. То есть полученные
объекты будут иметь ссылку на прототип конструктора.*/
function Car(name) {
    this.name = name;
...
прототипе указать как метод, так и это свойство, то после создания объекта на основе этого конструктора этот объект не
будет иметь этого свойства, но это свойство будет в прототипе. И если затем попытаться прочитать это свойство у этого
объекта, например, при помощи "console.log()", то внутри этого объекта свойство найдено не будет, но будет найдено в
...
/*Если в конструкторе не указать какое-то свойство и метод, в котором это свойство создается или изменяется в виде
объекта, а в прототипе указать как метод, так и это свойство, то после создания объекта на основе этого конструктора
