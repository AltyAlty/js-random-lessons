...
function Player(
    ...
    this.predictCloseCollision = function () {
        ...
        for (let i = 0; i < walls.length; i++) { // Перебираем каждую стену.
            ...
            if (helper.checkIntersectionBetweenTwoNotRotatedRectangles(
                ...
                проекцию игрока, которая будет в следующем тике, ближе к текущей позиции игрока на 1 до тех пор, пока не
                пропадет коллизия между предполагаемой позицией игрока и какой-то стеной. Делаем это пока игрок
                активный.*/
                ...
                ближе к текущей позиции игрока, то это будет означать, что мы имеем самую близку позицию игрока для
    ...
    this.predictFarCollision = function () {
        ...
        /*Создаем переменные, которые будут хранить расстояния до препятствий, которые могут потенциально оказаться на
        пути, который игрок пройдет за следующий тик.*/
    ...
    this.move = function () {
        if (this.isActive) { // Реализуем движение игрока, если он активен.
            ...
            if (this.currentSpeedY < this.gravity && this.currentAccelerationY !== 0) { // Если мы прыгаем,
                ...
                запрещаем игроку дальше прыгать и сбрасываем расстояние, которые мы прошли в воздухе во время прыжка,
            ...
            мы хоть немного прыгнули и опустились на поверхность какой-то стены, то сбрасываем расстояние, которое прошли
            в воздухе во время прыжка, чтобы в следующем прыжке начать отсчет сначала.*/
    ...
    this.drawGIF = function () {
        ...
        if (this.isActive) {
            ...
            следующий кадр оказался самым первым из раскадровки. Иначе, двигаем по раскадровке дальше с указанием во
