...
/*Метод "getImageData()" возвращает объект "ImageData", представляющий базовые данные пикселей для указанной части
холста. На этот метод не влияет матрица преобразования холста. Если указанный прямоугольник выходит за пределы холста,
пиксели за пределами холста становятся прозрачными черными в возвращаемом объекте "ImageData".
...
В этом примере рисуется изображение, а затем используется метод "getImageData()" для захвата части холста. Мы этот метод
для извлечения фрагмента изображения, начиная с (10, 20), с шириной 80 и высотой 230. Затем мы рисуем этот фрагмент три
раза, последовательно располагая фрагменты ниже и правее последнего кусочка.*/
