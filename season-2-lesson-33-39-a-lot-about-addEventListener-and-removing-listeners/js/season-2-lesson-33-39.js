/*Еще одна реализация отписки сразу всех слушателей от какого-либо элемента. Этот код
должен быть в самом начале.*/

/*Сначала мы модернизируем работу оригинального метода "addEventListener()" так,
чтобы у каждого элемента события хранилось отдельное свойство, содержащее информацию
о своих слушателях.*/
...
/*Затем мы создаем специальную функцию зачистки слушателей, которая как
очищает указанных в HTML слушателей событий, так и слушателей и специального
свойства, созданного нами выше в каждом элементе события.*/
...
/*Чтобы отписать сразу всех слушателей от какого-либо элемента, можно клонировать этот элемент.
Но в данной реализации мы также отписываем всех слушателей всех дочерних элементов указанного элемента.
Это не самый оптимальный способ, он медленный и затратный для памяти.*/

/*Метод "cloneNode()" возвращает дубликат узла, из которого этот метод был вызван.
В качестве параметра указывается должны ли дети узла быть клонированы или нет. Если нет,
то эти дочерние элементы при замене указанного элемента будут утеряны.*/
...
/*Еще одна реализация отписки сразу всех слушателей от какого-либо элемента, но в этот раз
можно указывать должны ли быть отписаны также все слушатели всех дочерних элементов указанного
элемента.*/
function removeAllListenersFromElementAndOrItsChildren(element, withChildren) {
  if (withChildren) {
    ...
    Если данный дочерний элемент является ссылкой на существующий узел в документе, то метод
    "appendChild()" перемещает его из текущей позиции в новую позицию (нет необходимости удалять узел 
    из родительского узла перед добавлением его к какому-либо другому узлу).*/
    /*Свойство "firstChild" это свойство только для чтения, возвращающее первый потомок узла в древе или null, 
...
/*
Вызов метода "addEventListener()" со слушателем в виде анонимной функции создает
нового слушателя каждый раз. Вызов метода "removeEventListener" в такой ситуации
ничего не делает, поскольку анонимная функция создает уникальный объект каждый раз,
когда ее вызывают, и не происходит отсылки к уже существующему объекту.

Следующая реализация отписки сразу всех слушателей от какого-либо элемента позовляет
работать со слушателями в виде анонимных или стрелочных функций.
*/

/*Создаем объект, который предназначен для хранения слушателей.*/
var _eventListeners = {};

/*Делаем специальную функцию-обвертку для метода "addEventListener()".
В качестве параметров принимает узел, где должно произойти событие, тип события, 
слушателя этого события и опция "capture" для метода "addEventListener()".*/
const addListener = (node, event, listener, capture = false) => {
  /*Если указанного события в параметрах еще нет в объекте, предназначенном
  для хранения слушателей, то добавляем в этот объект свойство, имя которого
  будет как названия события, а значение будет пустым массивом, который
  в дальнейшем будет хранить самих слушателей.*/
  if (!(event in _eventListeners)) {
    _eventListeners[event] = [];
  };

  /*Далее добавляем в это свойство элемент массива в виде объекта, который содержит
  узел, в котором должно произойти событие, самого слушателя этого события и опцию 
  "capture" для метода "addEventListener()".*/
...
/*Делаем специальную функцию-обвертку для метода "removeEventListener()".
В качестве параметров принимает узел, где должно произойти событие, и тип события.*/
const removeAllListeners = (targetNode, event) => {
  /*Удаляем слушателей указанного события из указанного узла.
  Сначала мы отфильтровываем только те объекты в объекте, предназначенном
  для хранения слушателей, в которых узел совпадает с указанным узлом.
  А затем у каждого полученного объекта мы берем свойство, хранящее узел и
  вызываем для него метод "removeEventListener()" для совершения отписки.*/
  _eventListeners[event]
    .filter(
  ...
  /*Обновляем объект, предназначенный для хранения слушателей.
  В этом объекте мы по свойству, означающему указанный тип события,
  оставляем только те узлы, которые не равны указанному узлу.*/
...
/*Еще одна реализация отписки сразу всех слушателей от какого-либо элемента и всех его 
дочерних элементов. Достаточно простой, но грубый вариант, медленно работает на больших 
страницах. Атрибут "outerHTML" получает сериализованный HTML-фрагмент, описывающий элемент, 
включая его потомков. При помощи этого атрибута можно установить замену элемента узлами, 
полученными из заданной строки.*/
setTimeout(() => {
