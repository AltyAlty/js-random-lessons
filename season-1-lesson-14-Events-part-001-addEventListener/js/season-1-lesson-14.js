...
showPassElement[0].addEventListener('mousedown', showMyPass); /*Событие "mousedown" означает зажатие какого-то
HTML-элемента мышкой.*/
showPassElement[0].addEventListener('mouseup', hideMyPass); /*Событие "mouseup" означает отсутствие зажатия
какого-то HTML-элемента мышкой.*/
...
hideSomeWindowElement[0].addEventListener('click', shrinkSomeWindow); /*Подписываем "hideElement" на событие
"onClick". При срабатывании такого события сработает функция "shrinkSomething".*/
showSomeWindowElement[0].addEventListener('click', unshrinkSomeWindow); /*Подписываем "showElement" на событие
"onClick". При срабатывании такого события сработает функция "unshrinkSomething".*/
...
function showStatusInput() {
    ...
    statusInputElement[0].value = statusSpanElement[0].innerText; /*У HTML-элемента "span" надо обращаться к свойству
    "innerText", а не "value".*/
};
...
checkNumbersInputElement[0].addEventListener('input', function () { /*Событие "input" означает ввод
чего-либо в HTML-элемент "input".*/
...
var mainMenuElement = document.querySelector(mainMenuClass); /*Таким образом получаем весь HTML-элемент с
классом "main-menu" и все его дочерние элементы. При использовании "querySelector()" класс нужно указывать через
точку.*/

mainMenuElement.addEventListener('click', addShowSubMenuClass);

function addShowSubMenuClass(event) {
    var menu = event.target.closest(mainMenuItemClass); /*Таким образом находим ближайший родительский элемент
    (или сам элемент), который соответствует заданному CSS-селектору. То есть при нажатии на HTML-элемент "span" находим
    его родительский HTML-элемент "li" c классом "main-menu-item". При использовании "closest()" класс нужно указывать
    через точку. Здесь "event" нужно передавать как параметр, иначе будет использоваться глобальный объект "event", что
    является нежелательным и устаревшим.*/
    menu.classList.toggle(showSubMenuClass); /*Таким образом можно добавлять/удалять какой-то класс у HTML-элемента. То
    есть здесь мы добавляем/удаляем класс "show-sub-menu" к родительскому по отношению к HTML-элементу "span", по
    которому мы кликнули, HTML-элементу "li" c классом "main-menu-item".*/
};
...
/*Таким образом получаем все HTML-элементы "span" внутри HTML-элементов с классом "main-menu-item-two".*/
var allSpansInMainMenuItemTwoArray = document.querySelectorAll(allSpansInMainMenuItemTwoClass);

for (var i = 0; i < allSpansInMainMenuItemTwoArray.length; i++) { /*Подписываем каждый HTML-элемент "span" на
событие.*/
    allSpansInMainMenuItemTwoArray[i].addEventListener('click', addShowSubMenuTwoClass);
};

function addShowSubMenuTwoClass() {
    /*Пробегаемся по всем HTML-элементам "span".*/
    for (var i = 0; i < allSpansInMainMenuItemTwoArray.length; i++) {
        if (allSpansInMainMenuItemTwoArray[i] === this) { /*Если нашли тот, на который нажали, то находим для него
        ближайший родительский HTML-элемент с классом "main-menu-item-two" и добавляем этому элементу класс "show-sub-menu-two".*/
            this.closest(mainMenuItemTwoClass).classList.add(showSubMenuTwoClass);
        } else { /*Иначе если нашли не тот, на который нажали, то находим для него ближайший родительский HTML-элемент с
        классом "main-menu-item-two" и удаляем у этого элемента класс "show-sub-menu-two".*/
