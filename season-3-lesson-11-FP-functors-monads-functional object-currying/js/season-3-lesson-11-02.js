...
/*Сделаем еще один функциональный объект. Слудующая программа получает значение, которое
показывает, сколько свойств может быть в объекте, а затем этот объект заполняется свойствами.
При достижении максимального количества свойств в этом объекте срабатывает callback.*/
function Collector() { };
...
const dataCollectorFour = collect(4).done((err, data) => {
    ...
    /*Метод "console.dir()" отображает список свойств указанного объекта. Вывод представлен в виде иерархического списка с
    возможностью просмотра содержимого дочерних объектов. Не стандартизировано!*/
