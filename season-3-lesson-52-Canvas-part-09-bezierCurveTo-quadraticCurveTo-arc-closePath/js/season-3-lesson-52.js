...
/*Метод "bezierCurveTo()" добавляет кубическую кривую Безье к контуру. Для построения требуются три точки. Первые 
две точки являются контрольными, а третья - конечной. Начальной точкой является последняя точка в текущем 
контуре, и она может быть изменена методом "moveTo()" перед созданием кривой Безье.*/
...
— контрольная, а вторая — конечная. Начальная точка — это последняя точка на текущем пути, которую можно изменить с
...
/*Метод "closePath()" добавляет в путь линию от текущей до первоначальной точки под пути и закрывает сам путь 
(давая возможность инициализировать новый). Если путь пустой или содержит одну точку, этот метод ничего не 
делает.*/
ctx.closePath();
