...
1. Добавить в объект метод под символьным ключом "[Symbol.iterator]", который всегда должен возвращать объект, в
котором в свою очередь должен быть всегда метод "next()". Разрешена дополнительная логика в методе 
"[Symbol.iterator]()" и объекте, которым будет возвращен этим методом.

2. Метод "next()" всегда должен возвращать один из двух объектов: {done: false, value: any value} или {done: true}.
Первый объект будет говорить циклу "for..of", что работа цикла "for..of" еще не завершена (done: false) и нужно взять 
указанное значение в текущей итерации (value: any value). Второй объект будет говорить, что работа цикла "for..of"
завершена (done: true) и никаких значений больше не надо получать. Разрешена дополнительная логика в методе "next()".

3. Когда цикл "for..of" начнет свою работу, он попытается вызвать один раз метод "[Symbol.iterator]()". Если такой
метод будет в итерируемом объекте и этот метод вернет другой объект, в котором будет метод "next()", то после этого 
шага цикл "for..of" будет работать только с этим последним объектом, полученным от метода "[Symbol.iterator]()". 

4. Далее цикл "for..of" на первой итерации попробует вызвать метод "next()" в полученном от метода 
"[Symbol.iterator]()" объекте и в зависимости от полученного объекта от этого метода "next()" будет действовать 
дальше. Если будет получен объект типа {done: false, value: any value}, то будет передано какое-то значение для 
текущей итерации и запущен метод "next()" еще раз для запуска следующей итерации. Если же будет получен объект типа 
{done: true}, то цикл "for..of" завершит свою работу.

5. Как именно будет метод "next()" итерировать свойства перебираемого объекта зависит от программиста. Мы свободны в 
написании логики метода "next()", пока мы соблюдаем условие, что в конце своей работы этот метод возвращает один из 
двух объектов: {done: false, value: any value} или {done: true}. Например, выше наш объект имеет только два свойства, 
которые представляют собой границы диапозона чисел (от 1 до 6), и по нашей задумке мы хотим проитерировать по всем 
числам этого диапозона. Соответственно, наш метод "next()" на основе граничных чисел диапозона, должен сам воссоздать 
остальные числа этого диапозона (2, 3, 4, 5). Поэтому помимо указанного выше требования по возращению одного из двух 
объектов, мы реализовали в методе "next()" дополнительную логику по генерации остальных чисел диапозона. 
...
"[Symbol.iterator]()" для достижения наших целей, естественно не забыв про обязательное требование, чтобы метод 
"[Symbol.iterator]()" возвращал объект с методом "next()".*/
...
один итератор – сам итерируемый объект, а не отдельно возвращенный объект методом "[Symbol.iterator]()".*/
...
/*Можно сделать бесконечный итератор, используя "Infinity". Или можно создать итерируемый объект, который генерирует 
бесконечную последовательность псевдослучайных чисел. Метод "next()" не имеет ограничений в этом плане. Всегда можем 
прервать цикл, используя "break".*/
...
while (true) {
    ...
    /*Можно увидеть, что метод "next()" будет возращать один из двух объектов: {done: false, value: any value} или 
    {done: true}.*/
    console.log(result);
...
которых есть индексы и свойство "length", то есть, они выглядят как массивы. Строки это и то и то. Объекты выше это 
только итерируемые объекты, но не псевдомассивы. Но можно сделать объект, который является псевдомассивом, но его нельзя
итерировать.*/
let obj3 = {
    0: 'a',
    1: 'b',
...
/*Полный синтаксис метода "Array.from()" позволяет указать необязательную "трансформирующую" функцию в качестве второго
параметра: "Array.from(obj[, mapFn, thisArg])". Второй необязательный параметр может быть функцией, которая будет 
применена к каждому элементу перед добавлением в массив. Также можно указать третий необязательный параметр "thisArg",
который позволяет установить контекст "this" для этой функции.*/
let obj6 = {
    a: 1,
    b: 4
};
...
/*Суррогатные пары это символы, которые записываются двумя 16-битными словами, поэтому их длина 2. Можно создать свою
версию метода "slice()", которые может работать с суррогатными парами.*/
