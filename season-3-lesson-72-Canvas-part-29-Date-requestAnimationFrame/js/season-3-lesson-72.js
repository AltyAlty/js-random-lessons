...
function draw() {
    ...
    // Земля
    /*При помощи конструктора "Date()" можно создать специальный объект, содержащий число миллисекунд прошедших с 1
    января 1970 г. UTC.*/
    let time = new Date();
...
function init() {
    ...
    запланировать перерисовку на следующем кадре анимации. В качестве параметра метод получает функцию, которая будет
    вызвана перед перерисовкой.*/
    window.requestAnimationFrame(draw);
};
