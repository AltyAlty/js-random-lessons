...
3. В функциях не должно быть поочных эффектов - обращений к глобальным переменным, запросов на сервер,
асинхронной работы и так далее.

ФП основывается на лямбда-вычислениях. Лямбда вычисления - это формальная система, которая решает вопрос
вычислимости алгоритма и является аналогом машины Тьюринга, то есть чтобы определить, что алгоритм будет
работать, какое время будет работать и будет воспроизводим на вычислительной технике. Лямбда-функция - это
любая безымянная функция.
*/
/*-------------------------------------------------------------------------------------------------------------*/
...
/*Функтор - это функция, которая получает какой-то набор значений и какую-то функцию, распаковывает набор
значений для получения этих значений, вызывает полученную функцию с каждым из этих значенияй, упаковывает 
возвращенные значения в новую структуру, и в конце возвращает эту структуру. При этом возвращенная структура 
не обязательно должна иметь тот же тип, что и оригинальная. Метод "map()" является функтором.*/
...
/*Если рассматривать строку как коллекцию символов, то можно написать функтор для строк. Символы имеют коды, 
которые на деле являются числом. Поэтому, мы можем выполним функцию "sumSomethingAndSix()" на каждом коде 
символа, и запаковать обратно в строку каждый полученный символ.*/
...
function functorForStrings(value, func) {
    ...
    параметра можно определить символы, разделяющую элементы внутри строки. Если параметр опущен, то возвращённый 
    массив будет содержать один элемент со всей строкой. Если параметр равен пустой строке, то строка будет 
    преобразована в массив символов.*/
    let chars = value.split("");
    console.log(chars); // ["A", "B", "C"]

    return chars.map(function (char) {
        /*Метод "String.fromCharCode()" возвращает строку, созданную из указанной последовательности
        значений единиц кода UTF-16.*/
        /*Метод "charCodeAt()" возвращает числовое значение Юникода для символа по указанному индексу (за 
        исключением кодовых точек Юникода, больших 0x10000).*/
        /*Метод "join()" объединяет все элементы массива (или массивоподобного объекта) в строку. В качестве
        параметра можно определить строку, разделяющую элементы массива.*/
        /*То есть здесь мы сначала получаем элемент массива, например, строку "A". Затем получаем ее
        "номер" - 65. Затем к этому номеру прибавляем 6, получаем 71. После этого находим символ под кодом
        71 - "G". И так далее с каждым элементом массива. В итоге промапив исходный массив, мы превращаем
        новый массив в строку.*/
...
/*Функтор, сделанный при помощи замыкания, безопаснее функторов, сделанных на классах или прототипах, так как
во втором случае, то значение, которое будет храниться в этом функторе, у всех к нему доступ.*/
...
/*Сначала создается первый объект с x = 5. Затем у первого объекта вызываем метод "map()" без аргументов, 
как результат будет создан второй объект с x = null. Затем уже у второго "пустового" объекта вызываем 
метод "map()", передав в качестве параметра функцию, следовательно в консоли будет пусто, так как снова будет
создан уже третий объект с x = null.*/
new CanWeUseItTwo(5).map().map(console.log);

/*Сначала создается первый объект с x = 5. Затем у первого объекта вызываем метод "map()", передав в качестве 
параметра функцию, как результат создаем второй объект с x = 10. Затем уже у второго объекта вызываем метод 
"map()", передав в качестве параметра другую функцию, следовательно в консоли будет 10, так как будет создан 
уже третий объект с x = 10.*/
...
параметра функцию, как результат создаем второй объект с x = null. Затем уже у второго "пустового" объекта 
вызываем метод "map()", передав в качестве параметра другую функцию, следовательно в консоли будет пусто, так 
как будет создан уже третий объект с x = null.*/
...
4. Возвращается еще один вызов функции "canWeUseItThree()", параметр которой высчитывается как 5 => 5 * 2, то 
есть x = 10.
5. Затем вызывается функция внутри с параметром в виде функции "x => ++x" (дело происходит уже во втором вызове 
функции "canWeUseItThree()").
6. Поскольку у нас снова есть и "x" и "func", то попадаем в первую ветвь if-блока.
7. Возвращается еще один вызов функции "canWeUseItThree()" (третьий по счету), параметр которой высчитывается 
как 10 => ++10, то есть x = 11.
...
10. Возвращается еще один вызов функции "canWeUseItThree()" (четвертый по счету), параметр которой высчитывается 
как "console.log(11)", то в консоли отобразится 11. Поскольку "console.log()" ничего не возвращает, то параметром
четвертового вызова функции "canWeUseItThree()" будет "undefined".
11. Четвертый вызов функции "canWeUseItThree()" по сути ничего не делает и завершает свою работу, так как
вызова функции внутри мы уже не делаем. 
...
распаковать функтор со значением, фунтор с функцией, применить функцию к значению, полученный результат упаковать
в функтор.*/
...
CanWeUseItSix.prototype.map = function (func) {
    /*Если есть "x" и была передана функция "func", то вызываем эту функцию с параметром в виде "x", иначе 
    "null."*/
    const result = (this.x && func) ? func(this.x) : null;
    // console.log(result);

    /*Оператор "instanceof" проверяет, принадлежит ли объект к определенному классу. Если в предыдущей строке
    мы уже получили объект класса "CanWeUseItSix", тогда его и возвращаем, иначе создаем объект, где
    в качестве "x" будет значение "result".*/
...
CanWeUseItSix.prototype.appply = function (functor) {
    /*Здесь мы сначала из текущего функтора вызываем метод "map()", чтобы получить значение "value". И дальше
    это "value" попадает в другой функтор, и уже вызываем метод "map()" у этого другого функтора, чтобы получить
    из этого функтора функцию, чтобы в итоге вызвать эту функцию со значением "value".*/
...
/*"canWeUseItSixInstOne" это объект с "x = 5". Сначала у этого объекта мы вызываем метод "appply()" и передаем
в него параметр в виде "functorOne" - объекта, где "x" это функция "x => x * 2". Что происходит внутри этого
метода "appply()":
...
8. Далее в последнем указанном куске кода мы должны выполнить метод "map()" у "functor", то есть у объекта
"functorOne", передав параметр "f => f(5)".
9. В этом втором вызове метода "map()" мы находим "result".
10. Поскольку в этом втором вызове метода "map()" "this.x" будет равен функции "x => x * 2", а "func" будет 
равен "f => f(5)", то мы выполняем "func(this.x)", то есть "(x => x * 2) => (x => x * 2)(5)". Грубо говоря 
мы должны в итоге вызвать функцию "x => x * 2" с параметром "5" и получить 10.
11. Затем в этом втором вызове метода "map()" возвращается "result".
12. Так как "result" равен "10", то он не является экземпляром "CanWeUseItSix", поэтому мы создаем
новый объект класса "CanWeUseItSix" c "x = 10".
13. Далее этот новый созданные объект получает наш первый вызов метода "map()", который мы делали в 
объекте "canWeUseItSixInstOne".
...
16. Поскольку в этот раз "result" уже является экземпляром класса "CanWeUseItSix", то мы ничего не делаем
с этим "result" и отдаем этот объект наружу дальше методу "appply()".
17. Метод "appply()" в свою очередь возвращает дальше наружу полученный объект класса "CanWeUseItSix" с
"x = 10".

18. Далее у этого объекта мы делаем второй вызом метода "appply()", передав в него объект "functorTWo",
то есть объект, где x это функция "x => ++x".
19. Далее повторяются шаги аналогичные шагам 1-17.
20. Главное отличие здесь будет в том, что в один момент мы будем использовать функцию "x => ++x" вместо 
"x => x * 2" и применять мы будем эту функцию уже над "10", а не на "5".
...
24. Поскольку в этом вызове метода "map()" "this.x" будет равен "11", а "func" будет равен 
"console.log", то мы выполняем "func(this.x)", то есть "console.log(11)".
...
28. Поскольку в этот раз "result" не является экземпляром класса "CanWeUseItSix", поэтому мы создаем
новый объект класса "CanWeUseItSix" c "x = undefined".
29. В итоге этот объект возвращается методом "map()" наружу.
30. Поскольку в этом объекте "x = undefined", то при дальнейшем применении методов "appply()" и "map()"
мы будем получать новые объекта класса "CanWeUseItSix" c "x = null", соотвественно в консоли мы ничего
не увидим больше.
*/
...
/*Добавляем в наш объект метод "mapNull()", который на входе получает какую-то функцию и какое-то значение. Если
это какое-то значение было передано, то применяем полученную функцию к этому значению.*/
fp.mapNull = (func, x) => (x ? func(x) : null);

/*Добавляем в наш объект метод "canWeUseIt()", который на входе получает какое-то значение, попадающее в
дальнейшем в замыкание. Внутри этого метода мы создаем функцию "map()". Далее к этой функции мы создаем еще
две функции "appply()" и "chain()". Функции "map()" и "appply()" это аналоги одноименных методов из предыдущего
примера, а функция "chain()" нужна для осуществления применения функций по цепочке. В итоге этот метод 
"canWeUseIt()" возвращает функцию "map". Здесь получается, что:
...
3. функция "chain()" позволяет применять функции, которые возвращают функторы, к значению в другом функторе,
то есть грубо говоря позволяет делать цепочки вычислений.
...
/*
Здесь мы обращаемся к функции "map()", где везде "x" равен "10", и вызываем у нее функцию "appply()", которая 
должна выглядеть примерно как "map.appply = funcA => funcA(func => fp.mapNull(func, 10));".

Мы вызываем эту функцию "appply()" с параметром "fp.canWeUseIt(x => ++x)", поэтому мы должны сначала узнать чему 
будет равен этот параметр. То есть мы должны сначала вызвать функцию "fp.canWeUseIt()" с параметром в виде 
функции "x => ++x".

В итоге эта функция "fp.canWeUseIt()" вернет функцию "map()", где везде "x" равен функции "x => ++x". Получается,
что параметром для функции "appply()" станет другая функция "map()", где везде "x" равен функции "x => ++x". 

Нужно помнить, что мы вызываем эту функцию "appply()" в контексте функции "map()", где везде "x" равен "10". 
Значит в строке кода "map.appply = funcA => funcA(func => fp.mapNull(func, 10));" "funcA" будет равен функции 
"map()", где везде "x" равен функции "x => ++x". То есть мы должно вызвать функцию 
"map = func => fp.canWeUseIt(fp.mapNull(func, x => ++x));" с параметром в виде функции 
"func => fp.mapNull(func, 10)", то есть примерно следующее:
...
"func => fp.mapNull(func, 10)" и параметром "x" равным функции "x => ++x". Внутри функции "fp.mapNull()" мы 
вызовем функцию "func => fp.mapNull(func, 10)" с параметром в виде функции "x => ++x", то есть примерно это - 
"x => ++x => fp.mapNull(x => ++x, 10)". Это означает, что мы вызываем еще одну функцию "fp.mapNull()" с параметром 
"func" равным функции "x => ++x" и параметром "x" равным "10", что в итоге вернет нам "11". Получается, что 
предыдущий вызов функции "fp.mapNull()" получит это "11" и вернет это "11" наверх ввиде параметра для функции 
"fp.canWeUseIt()". Эта функция "fp.canWeUseIt()" вернет функцию "map()", где везде "x" равен "11", что в итоге и 
сохранится в переменную "step3".
*/
// let step4 = step3(console.log); // В консоль выводим 11. Вернет функцию "map()", где везде "x" равен "undefined".                                
