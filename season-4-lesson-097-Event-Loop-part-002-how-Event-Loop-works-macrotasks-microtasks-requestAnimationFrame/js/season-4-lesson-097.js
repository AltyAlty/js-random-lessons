...
"rendering pipeline". Браузер ждет момента перед следующей фазой - фазой рендеринга, параллельно синхронизируясь с
...
Именно в этот момент браузер проверяет специальную очередь callback-функций, переданных функции "requestIdleCallback()" 
- "idle callback queue". Если пользователь начал взаимодействие со страницей, например клик или скролл, то браузер 
прерывает выполнение idle-задач и возвращается к ним позже.
...
4. Другие API:
    ...
    3) Callback-функции экземпляра "IntersectionObserver", которые вызываются при пересечении элементов с viewport.*/
...
1. Callback-функции обработчиков промисов, например:
    1) Метода "then()".
    2) Метода "catch()".
    3) Метода "finally()".
2. Callback-функции функции "queueMicrotask()".
3. Callback-функции экземпляра "MutationObserver", которые вызываются при отслеживании изменений DOM.*/

/*--------------------------------------------------------------------------------------------------------------------*/

/*"MutationObserver" - это встроенный API браузеров, который позволяет отслеживать изменения в DOM (в структуре,
...
//         attributes: true,
//         /*Отслеживаем только конкретные атрибуты. Свойство "attributeFilter" работает только для встроенных атрибутов.*/
//         // attributeFilter: ['class', 'customAttribute']
//     }
// );

// /*3. Пример изменения - будет вызвана callback-функция.*/
// element.setAttribute('customAttribute', '456');
...
/*Почему функция "requestAnimationFrame()" работает плавно, даже если выполняется после макрозадач:
...
/*Почему функция "requestAnimationFrame()" не тормозит, если она идет после микрозадач:
