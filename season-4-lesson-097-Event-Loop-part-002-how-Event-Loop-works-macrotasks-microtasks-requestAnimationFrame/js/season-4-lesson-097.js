...
Call stack - единственное место, где выполняется код (будь то синхронный или асинхронный колбэк).*/

/*--------------------------------------------------------------------------------------------------------------------*/

/*Шаги работы Event Loop:

1. Фаза запуска скрипта. На этой фазе выполняется весь доступниый синхронный код. Этот синхронный код выполняется только
...
вызова метода "then()" или "catch()" не регистрируются все сразу, каждый ждет выполнения предыдущего. Каждый новый
...
- Изменились визуальные свойства, например, "color", "background-color", background" или "box-shadow".
...
Что такое функция "requestIdleCallback()"?
Это API, которое:
- Ставит задачу в очередь, но выполняет ее только в периоды простоя.
- Гарантирует, что задача не помешает критически важным процессам, например, рендерингу, анимациям, вводу пользователя.
...
7. На этом текущий цикл заканчивается и начинается новый цикл, в котором повторяются шаги 2 - 7. Если очередь макрозадач
пуста (шаг 2), то браузер может "заснуть" до появления новых задач, например, событий мыши или таймеров.
...
5. Другие API:
    ...
    окнами или фреймами через свойство "postMessage" окна или фрейма.
...
2. Функция "requestAnimationFrame()" и Web Workers:
    ...
    результаты передавать в функцию "requestAnimationFrame()" через свойство "postMessage" окна или фрейма.
